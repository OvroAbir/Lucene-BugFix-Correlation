...
class Jira_Issue:
	...
	def __calculate_fix_times(self):
		...
		status_event_times = self.__get_all_event_times(issue_creation_time, issue_history)
	...
	def __get_all_event_times(self, histories):
		...
		for history in histories:
			for item in history.items:
				if item.filed == 'status':
					status_event_times.push((history.created_time, item.from_str, item.to_str))
